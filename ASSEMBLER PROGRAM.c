...
int startaddress,loc_ctr,sym_count=0,length,f=0;
char line_1[20],lbl[8],opcode[8],operand[8],programmename[10];
          ...
          void ps_2()//START OF PASS 2:------------------------------------
          {
               ...
               printf("\nH^ %s ^ %d ^ 00%d \n",programmename,startaddress,length);//FOR HEADER RECORD
               fprintf(output,"\nH^ %s ^ %d ^ 00%d \n",programmename,startaddress,length);
            ...
            for(check1=0;check1<3;check1++)
             {
               if(!strcmp(opcode,myopcodetab[check1].cde))
               {
               fndopcode=1;
              ...
              if(!foundoperand)strcat(part,"err");
               ...
               if((currenttxtln+strlen(part))<=8)
               {
                ...
                else
                {
                    if(f==0)
                 {
                     printf(" T^ %d %s",recaddress,record);//FOR TEXT RECORD
                 fprintf(output," T^ %d %s",recaddress,record);
                 ...
                 f=1;
                    ...
                    printf(" ^ %d %s",recaddress,record);//FOR TEXT RECORD
                 fprintf(output," ^ %d %s",recaddress,record);
                 recaddress+=currenttxtln/2;
                 currenttxtln=strlen(part);
                 strcpy(record,part);
                 }
  ...
  void check_opcd()//CHECK FOR OPCODE TABLE
         {
          ...
          for(l=0;l<3;l++)
                          ...
                          if(!strcmp(opcode,myopcodetab[l].cde))
                          {
